...
entry_file=${ENTRY_FILE:-"${REPO_ROOT}/src/train/train_molmo2_sft.py"}
datasets=${DATASETS:-"physics_violation_sampled_train"}

run_name=${RUN_NAME:-"molmo2-4b-physics-violation-bs${batch_size}-ga${grad_accum_steps}-fps${VIDEO_FPS:-2}-maxf${VIDEO_MAX_FRAMES:-20}-minf${VIDEO_MIN_FRAMES:-10}-imgsize${MOLMO2_IMAGE_SIZE:-378}-lr${lr}"}
